...
func _on_hitbox_body_entered(body: Node3D) -> void:
	...
	if body is RigidBody3D and body.has_method("receive_damage_from"):
		...
		# Deal damage
		if target_id >= 9000 or not player.multiplayer.has_multiplayer_peer() or player.multiplayer.multiplayer_peer == null or player.multiplayer.multiplayer_peer.get_connection_status() == MultiplayerPeer.CONNECTION_DISCONNECTED or target_id == player.multiplayer.get_unique_id():
