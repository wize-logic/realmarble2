...
func activate() -> void:
	...
	DebugLogger.dlog(DebugLogger.Category.ABILITIES, "LIGHTNING STRIKE! Calling down thunder from above!", false, get_entity_id())
	...
	if not target:
		...
		if camera:
			...
			spawn_lightning_strike(strike_position, null)
		return

	# Strike at target's position (predict slightly)
	...
	spawn_lightning_strike(strike_position, target)

func spawn_lightning_strike(position: Vector3, target: Node3D) -> void:
	"""Spawn the lightning strike effect and deal damage"""
	if not player or not player.get_parent():
		return

	# Spawn warning circle at strike location
	spawn_warning_indicator(position)
	...
	if target and is_instance_valid(target) and target.is_inside_tree():
		...
		if distance_to_strike < 3.0:  # Within strike radius
			...
			# Strong upward knockback from lightning
...
func spawn_warning_indicator(position: Vector3) -> void:
	"""Spawn a warning circle at the strike location"""
	if not player or not player.get_parent():
		return
...
func spawn_lightning_bolt(position: Vector3) -> void:
	"""Spawn the dramatic lightning bolt effect"""
	if not player or not player.get_parent():
		return
	...
	# Create bright core flash at impact
	...
	impact_flash.light_energy = 15.0
	impact_flash.omni_range = 20.0
