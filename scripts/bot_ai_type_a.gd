...
## Bot AI Controller - TYPE A (Sonic-Style Arenas) v4.0
...
const RAIL_LAUNCH_RECOVERY_TIME: float = 6.0  # Max time to attempt recovery after launch (increased for better recovery)
const SAFE_LANDING_SEARCH_RADIUS: float = 50.0  # How far to search for safe landing zones (increased range)
...
func consider_rail_navigation() -> void:
	...
	# Try to attach if rail significantly helps reach chase target
		...
		# REDUCED: Only use rail if it's significantly better to avoid interrupting chase
		if dist_via_rail < dist_now * 0.8:
			should_attach = true

	# Try to attach if we're wandering and rail provides exploration/mobility
	elif state == "WANDER":
		# REDUCED: 15% chance to use rails while wandering to avoid over-reliance
		if randf() < 0.15:
			should_attach = true
	...
	if should_attach:
		...
		# REMOVED: Force application toward rails - this was interfering with state machine
		# Rails should only be used when bot is very close, not as navigation targets
	else:
		target_rail = null
	...
	if action_timer <= 0.0:
		if height_diff > 10.0:
			...
			if "jump_count" in bot and "max_jumps" in bot:
				...
				# Fallback: aggressive single jump
				bot_jump()
				action_timer = randf_range(0.2, 0.4)
...
func use_ability_smart(distance_to_target: float) -> void:
	...
	if should_use and should_charge and can_charge and not is_charging_ability:
		if bot.current_ability.has_method("start_charge"):
			...
			DebugLogger.dlog(DebugLogger.Category.BOT_AI, "Charging %s (%.1fs) | Dist: %.1fu | LOCKED ON" % [ability_name, ability_charge_timer, distance_to_target], false, get_entity_id())
	...
	if is_charging_ability and ability_charge_timer <= 0.0:
		...
		DebugLogger.dlog(DebugLogger.Category.BOT_AI, "Released %s | Dist: %.1fu | LOCK RELEASED" % [ability_name, distance_to_target], false, get_entity_id())
