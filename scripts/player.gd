...
var base_acceleration: float = 100.0  # Base acceleration rate - high for responsive feel
var current_acceleration: float = 100.0  # Level-scaled acceleration
var deceleration: float = 40.0  # Deceleration rate when no input (units/sec^2)
...
func _physics_process(delta: float) -> void:
	...
	if input_dir != Vector2.ZERO:
		...
		var min_force: float = current_roll_force * 0.4 * control_multiplier
